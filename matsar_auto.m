...
codes_im_cal = zeros(1,numsamp);
for n = 1:numsamp
    codes_ideal(n)  = Regs(n,:) * C(1:M)'/Cmin;
    codes_im_cal(n) = W(numsamp,:)*(Regs_mis(n,:) .* C(1:M) /Cmin)';
...
codes_cal = codes_im_cal;
if To10Bits
    codes_cal = round(codes_cal);
    codes(codes_cal < 0) = 0;
    codes(codes_cal > 1023) = 1023;
end
